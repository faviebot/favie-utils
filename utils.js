...
/**
 ...
 @param {string} url - The URL or import.meta.url.
 @returns {Object} - An object containing filename and dirname properties.
 */
export const __ = (url) => {
  const filename = new URL(url ?? import.meta.url).pathname
 ...
 */
export const isNull = (variable, strict = true) => {
  return strict
    ? variable === null
 ...
 */
export const isBuffer = (variable) => {
 ...
 */
export const isBase64 = (variable) => {
 ...
 */
export const ensureArray = (variable) => {
 ...
 */
export const getCleanObject = (object) => {
 ...
 */
export const getObjectWithoutProperties = (object, properties = []) => {
  return Object.keys(object).reduce((newObject, property) => {
 ...
 */
export const getObjectWithProperties = (object, properties = []) => {
  return Object.keys(object).reduce((newObject, property) => {
    if (properties.includes(property)) {
      newObject[property] = object[property]
    }
 ...
 */
export const getObjectsProperties = (objects, properties) => {
 ...
 */
export const takeObjectProperties = (object, properties) => {
 ...
 */
export const getObjectsIds = (objects) => {
 ...
 */
export const getDomain = (url, isCapitalize = false) => {
 ...
 */
export const objectToText = (object, depth = 5) => {
  if (!isBrowser()) {
 ...
 */
export const textToWords = (text, strict = true) => {
 ...
 */
export const trimChars = (string, chars) => {
  chars ??= [
    '.', ',', ':', ';', '?', '!', '\'', '"', '`', '*', '(', ')', '[', ']',
    '{', '}', '<', '>', '/', '|', '\\', '-', '—', '_', '=', ' '
  ]
...
const downloadFileBuffer = async (url) => {
  try {
    ...
    logger(error, 'downloadFileBuffer()', { url })
  }
}
 ...
 */
export const streamToFile = async (stream, filePath) => {
 ...
 */
export const fileToBase64 = async (file, url = false) => {
  try {
    return new Promise((resolve, reject) => {
 ...
 */
export const writeFile = async (filePath, data, options = { flag: 'a+' }) => {
  if (isBrowser()) {
    return false
  }
...
export const logger = (...args) => {
  ...
  if (logType === 'error') {
    ...
    logMessage += Array.isArray(log)
      ? `${log.map(item => isObject(item) ? objectToText(item) : item)
        .join('\n')}\n`
      : `${log}\n`
  }
 ...
 */
export const getCache = async (cache, key, callback, expiry = '+1year') => {
  try {
    if (cache.has(key)) {
 ...
 */
export const hasCache = (cache, key, expiry = '+1year') => {
 ...
 */
export const clearCache = (cache, expiry = '+1year') => {
 ...
 */
export const checkCacheDate = (cacheDate, expiry = '+1year') => {
 ...
 */
export const getModifiedDate = (input, initialDate = null) => {
 ...
 */
export const getThumbnail = async (file, resize, quality = 100) => {
  try {
 ...
 */
export const getErrorMessage = (type, insert = null) => {
  switch (type) {
    case '!method':
 ...
 */
export const run = async (callback, repeatTime = null, attempt = 1) => {
  try {
